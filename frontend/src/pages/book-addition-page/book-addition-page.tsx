...
export const BookAdditionPage = () => {
  ...
  const { data: bookIsbn, isLoading: isLoadingIsbn, isError: isErrorIsbn } = useGetBooksByIsbnIsbn(isbnQuery, { query: { retry: false, enabled:  isbnQuery.length === 17 } });
  const { data: bookTitleList, isLoading: isLoadingTitleList, isError: isErrorTitleList } = useGetBooksByTitleTitle(debouncedTitle, { query: { enabled:  debouncedTitle.length > 4 } });
    ...
    <PageWithWrapper>
      ...
      <Tabs defaultValue={BookAdditionTabs.Isbn}>
        ...
        <Tabs.Panel value={BookAdditionTabs.Isbn}>
          ...
          {isbnQuery && <Flex direction="column" gap="sm" align="center" className={styles.buttonWithDescription}>
